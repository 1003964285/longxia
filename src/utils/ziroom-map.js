var ZMap = function(id, center, level) {
	...
	this.id = id; //div的id
	this.level = level ? level : 16; //地图级别
...
ZMap.prototype = {
	init: function() { //全局初始化
		...
		this.map.addEventListener("click", function(e) {
			...
			console.log(point, points, "888888888888888")
			...
			//			var x = 64;
			//			for(var i = zMap.level; i < 19; i++) {
			//			  x *= 0.5;
			//		  }
			//			var label1 = new BMap.Label("第一区域", {
			//				position: new BMap.Point(points[0].lng / 2 + points[2].lng / 2, points[0].lat / 2 + points[2].lat / 2)
			//			});
			//			label1.setStyle({
			//				color: "#fff",
			//				fontSize: "16px",
			...
			//				fontWeight: "bold"
			//			});
			//			console.log(zMap.level)
			//			zMap.map.addOverlay(label1);
			//zMap.show(points)
		});
	},
	show: function(points) {
		var label1 = new BMap.Label("1000", {
			position: new BMap.Point(points[0].lng / 2 + points[2].lng / 2, points[0].lat / 2 + points[2].lat / 2)
		});
		label1.setStyle({
			color: "#fff",
...
//			if(this.level < 13) {
//				console.log("000000000000000000000", polyline)
//				polyline.hide()
//				return;
//			}
		}
